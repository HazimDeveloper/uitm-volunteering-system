...
if(isset($_GET['my_event'])) {
    ...
    $select_data_event = mysqli_query($connect,"SELECT *,event.status AS stat FROM event JOIN user_event ON event.id_event = user_event.id_event WHERE id_user= $id AND user_event.status != 'cancel'");
        ...
        <div class="">
            <div class="container">
                <div class="section-header text-center">
                    <p>Upcoming Events</p>
                    <h2>Be ready for our upcoming volunteering events</h2>
                </div>

                <a href="event.php" class="btn btn-primary mb-5">All Event</a>
                <a href="event.php?my_event" class="btn btn-primary mb-5">My Event</a>
                <div class="row">
                        ...
                        <div class="event-item">
                            ...
                            <div class="event-content">
                                <div class="event-meta">
                                    ...
                                    <p><i class="far fa-clock"></i><?= $row['time_start']?> - <?= $row['time_start']?></p>
                                    <p><i class="fa fa-map-marker-alt"></i><?= $row['place']?></p>
									<p><i class="fas fa-user"></i><?= $row['status']?></p>
                                </div>
                                <div class="event-meta">
                                    <h3><?= $row['title']?></h3>
                                    <p>
                                    ...
                                    if(isset($_GET['my_event'])){ ?>
                                         ...
                                         <!-- <input type="submit" class="btn btn-custom card-link" value="Already Join" disabled name="submit"> -->
                                         <a href="delete_event.php?id_event=<?=$row['id_event'] ?>" class="btn btn-danger" onclick="return confirm('Are Your Sure Want To Cancel To Join The Event')" class="btn btn-warning" style="margin-top: 0px;padding-top:10px;padding-bottom: 10px;">Cancel Join Event</a>
                   
                                         <!-- <input type="submit" style="margin-top: 17px;" value="Cancel" class="btn btn-warning" name="cancel"> -->
                                   <?php }else{
                                    if($row['status'] == "available") {  ?>
                                    
                                    <a href="update_event.php?id=<?=$row['id_event'] ?>" class="btn btn-dark text-light">Join Now</a>
                                        <!-- <input type="submit" class="btn btn-custom" value="Join Now" name="submit"> -->
                                    <!-- <a class="btn btn-custom" href="">Join Now</a> -->
                                <?php }else if($row['status'] == 'completed') { ?>
                                    <input type="submit" class="btn btn-success" value="Completed" name="submit" disabled >
                                   
                                    <?php  ?>
                                    <?php }else{?>
                                    <input type="submit" class="btn btn-dark" value="Full" name="submit" disabled >
